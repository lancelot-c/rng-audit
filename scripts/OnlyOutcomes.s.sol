/* DEPRECATED
 * This script is getting randomness from .rng files which we consider as an inferior solution as it is not scalable
 * Fuzzing on the other hand is a much more scalable way to obtain on infinite amount of randomness at a minimum cost
 * Please use OnlyOutcomes.t.sol instead which uses fuzzing
*/
    ...
    function run() public {
        ...
        for (uint32 d = 0; d < draws; d++) {
            ...
            if (d % onePercentProgress == 0) {
                console.logString(
                    string.concat(
                        Strings.toString(percentProgress),
                        "% (",
                        Strings.toString(d),
                        "/",
                        Strings.toString(draws),
            ...
            bytes memory simulatedChainlinkResponse =
                extractBytes(totalEntropy, d * entropyNeededPerDraw, entropyNeededPerDraw);
